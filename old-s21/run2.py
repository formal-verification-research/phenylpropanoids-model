...
# dir = raw_input("Path to the prism directory: ")
...
while runb.lower() != "y" and runb.lower() != "n":
    runb = raw_input("Run a PRISM model? (y/n) ")
if runb.lower() == "y":
    runb = True
else:
    runb = False
if runb:
    model = raw_input("Name of the PRISM model (.sm) to run: ")
    # prop = raw_input("Name of the property file (.csl) to run: ")
    output_directory = raw_input("(Optional) Name of the directory to write to: ")
    if len(output_directory) != 0 and (output_directory[-1] != '/' or output_directory[-1] != '\\'):
        output_directory += "/"
    sorm = ""
    while sorm.lower() != "s" and sorm.lower() != "m":
        sorm = raw_input("Enter an m to check the model, or s if the model is a simulation: ")
    if sorm.lower() == "s":
        simn = raw_input("(Optional) Number of simulations to run: ")
        catch_str_start = "Simulating: P=? [ F[0,"
        catch_str_end = "\n"
    else:
        catch_str_start = "Model checking: P=? [ F[0,"
        catch_str_end = " (value"
    memory = raw_input("(Optional) Enter the amount of memory to use for the model: ")

    propb = ""
    while propb.lower() != "y" and propb.lower() != "n":
        propb = raw_input("Create new properties file? (y/n) ")
    if propb == "y":
        tmax = int(raw_input("Final time: "))
        dt = int(raw_input("Time step to check: "))
        varname = raw_input("Variable to check: ")
        ...
        while varname != "":
            varn += 1
            cmax = int(raw_input("Maximum concentration to check for " + varname + ": "))
            dc = int(raw_input("Concentration step to check for " + varname + ": "))
            for t in range(0, tmax, dt):
                for c in range(0, cmax, dc):
                    props.write("P=?[F[0," + str(t+dt) + "] (" + varname + ">=" + str(c) + " & " + varname + "<" + str(c + dc) + ")]\n")
            varname = raw_input("Variable to check (press enter when finished): ")
        props.close()

        print("Created properties file of size " + str(os.path.getsize(propname)/1000) + "kb as prop" + runnumber + ".csl")
    else:
        propname = raw_input("Name of properties file to run (.csl): ")

    print("\nPRISM is running the model...\n")
    prism_raw_input ="prism"
    if sorm == "s":
    ...
    '''
    # print(prism_raw_input)
    prism_output = subprocess.check_output(prism_raw_input, universal_newlines=True, shell=True)
    # print(prism_output)
    '''
    prism_output = ""
    process = subprocess.Popen(prism_raw_input, stdout=subprocess.PIPE, universal_newlines=True, shell=True)
    while True:
    ...
    analyze = raw_input("Model successfully ran with output " + output_directory + "prismrun" + runnumber + ".txt\n" +
    ...
    modelname = raw_input("Name of previous model run to analyze: ")
    output_directory = raw_input("(Optional) Name of the directory to write to: ")
    if len(output_directory) != 0 and (output_directory[-1] != '/' or output_directory[-1] != '\\'):
        output_directory += "/"
...
while statb != 'y' and statb != 'n':
    statb = raw_input("Include statistical analysis with each data point? (y/n) ")
if statb == 'n':
    statb = False
else:
    statb = True
...
while graphb != 'y' and graphb != 'n':
    graphb = raw_input("Plot data? (y/n) ")
if graphb == 'n':
    graphb = False
else:
    graphb = True
